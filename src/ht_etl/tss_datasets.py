...
def file_mapping(dataset_id, filename, database, url, dataset_type, genes_ranges):
    ...
    for tu_dict in tu_json_list:
        ...
        closest_genes = dataset_dict.get('closestGenes', None)
        promoter = None
        if closest_genes:
            gene_id = utils.find_min_by(closest_genes, 'distanceTo')
            tu = utils.get_tu_by_gene_id(gene_id, database, url)
            if tu:
                promoter = utils.get_promoter(tu, database, url)
        dataset_dict.setdefault('promoter', promoter)
